...
def _assert_error_contains(artifact: dict[str, object], expected: str, library_dir: Path = LIBRARY_DIR) -> None:
    try:
...
def test_no_matching_path_uses_no_improvisation_fallback() -> None:
    artifact = dict(_cases()[0]["artifact"])
    artifact["movement_response_id"] = "waiting-hand"
    _assert_error_contains(artifact, "No approved Nexus Echo path")
...
def test_runtime_word_count_is_enforced() -> None:
    with tempfile.TemporaryDirectory() as directory:
        copy_dir = Path(directory) / "library"
        shutil.copytree(LIBRARY_DIR, copy_dir)
        path = copy_dir / "echo_paths.json"
